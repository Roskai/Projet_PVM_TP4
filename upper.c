...
void upper_hull(point *pts)
{
    ...
    */
	...
	int sender[1]; /* tid du dernier envoyeur */
	...
	for (size_t i=0; to_UH_nb>0 && i<NB_SLAVE; i++) // Envoi d'un sous-ensemble à chaque esclave (dans la mesure des sous-ensembles disponibles)
		send_pb(tids[i], depile_to_UH());

	int inactive_slave = 0; /* nombre d'esclaves inactifs (pour quitter la boucle) */
	...
	while (1) // Lorsque tout les UH de sous-ensemble ont été calculé
	{
		...
		master_send_point(&sender, depile_to_UH(), depile_to_UH()); // On envoie les deux ensembles à fusionner
	}
