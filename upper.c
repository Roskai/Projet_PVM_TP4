...
static point **to_UH;	 /* Le tableau de points à calculer l'UH */
static int to_UH_nb = 0; /* Nombre d'élements dans le tableau */
...
static int to_merge_nb = 0; /* Nombre d'élements dans le tableau */
...
static int to_merge_merged_nb = 0; /* Nombre d'élements ayant été fusionnés dans to_merge_status */
...
/**
 * Envoie d'un calcul par le maitre à un esclave :
 *   - tid : Id de l'esclave
 *   - pts : Chaine de point à calculer
 *   - pts2 : Chaine de point à fusionner, si non fusion mettre NULL
 *   - id : Identifiant de la chaine pour pouvoir récupérer sa position à la réception
 */
...
/**
 * Reception du résultat d'un calcul d'un esclave par le maitre
 * Et ajout de ce dernier dans le tableau des chaines de points à fusionner
 */
...
/*
 * Calcul de l'enveloppe convexe parallèle
 */
point *upper_hull(point *points)
{
	...
	for (size_t i = 0; i < pb_max && i < NB_SLAVE; i++)
	{ // Envoi d'un sous-ensemble à chaque esclave (dans la mesure des sous-ensembles disponibles)
#if AFFICHE
		int sl = depile_slave();
	...
	while (!solved() && slave_queue_nb != NB_SLAVE) // Lorsque tout les UH de sous-ensemble ont été calculé
	{
...
#endif
		// Vérification de la présence de chaines de points à fusionner
		int pos1 = -1;
		int pos2 = -1;
		for (size_t i = 0; i < pb_max; i++)
		{
		...
		{ // Tant qu'il y a des chaines de points à fusionner
#if AFFICHE
			int sl = depile_slave();
			...
			// Re-vérification de la présence de chaines de points à fusionner
			pos1 = -1;
			pos2 = -1;
			for (size_t i = 0; i < pb_max; i++)
			{
	...
	// Envoi du message de fin aux esclaves
	pvm_mcast(slave_queue, NB_SLAVE, MSG_END);
	pvm_exit();
